...
为了提高部署效率，将服务器做成了跨平台可docker的版本。

原版服务器依赖商业组件 **DevExpress** 且不支持跨平台，
因此连同windows管理界面一同剥离出去，另行提供了自研web管理后台。
...
services:
	zircon:
		...
		ports:
			- 192.168.0.3:7000:7000
			- 192.168.0.3:7080:7080
...
**端口 `7000` 是游戏服务端口，客户端连这里；**
**端口 `7080` 是web管理后台端口，浏览器可直接访问。**

成功运行后能看到类似下面的输出就表示运行成功：

```
皓石传奇三 v1.9.0.25852
免费开源的传奇三，技术交流进QQ群 915941142
客户端更新路径：
地图文件路径：./datas/Map/
最大连接数量限制：200
