...
namespace Zircon.Server.Models.Monsters
{
    public class HealerAnt : SkeletonAxeThrower
    {
        ...
        public void Heal(MapObject ob)
        {
            ...
            Stats tmp = new Stats();
            tmp.Values.Add(Stat.Healing,Stats[Stat.Healing]);
            tmp.Values.Add(Stat.HealingCap, Stats[Stat.HealingCap]);

            ob.BuffAdd(BuffType.Heal, TimeSpan.MaxValue, tmp, false, false, TimeSpan.FromSeconds(1));
        }
